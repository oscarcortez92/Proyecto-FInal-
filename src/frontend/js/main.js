...
class Main {
    ...
    handleEvent(ev) {
        ...
        let objetoEvento = ev.target; //saber que objeto produjo el evento
        ...
        if (ev.type == "click" && objetoEvento.id.substring(0, 2) == "ck") {
            ...
            let idBuscar = this.getELement("id_buscar");
    ...
    handlerGetResponse(status, response, tipo) {
        console.log("LLEGO ", response);
        let respuestaObjetos = JSON.parse(response);
